...
const SearchResultsMain = () => {
  ...
  // Keep --search-sticky-height CSS variable in sync with the actual sticky bar height
  // so the sidebar's top: calc(66px + var(...) + 16px) is always accurate.
  useEffect(() => {
    const el = stickyTopRef.current;
    if (!el) return;
    const ro = new ResizeObserver(([entry]) => {
      document.documentElement.style.setProperty(
        '--search-sticky-height',
  ...
  // On page reload / direct URL access, the auto-search effect dispatches the
  // search shortly after mount. Until that happens, `searchLoading` is still
  // false and `searchResults` is null — without this guard the "no flights"
  // empty state would flash briefly. We default to `true` so SSR and the first
  // client render are identical (no hydration mismatch); an effect then
  // resolves the real value once the URL is available.
  const [hasPendingUrlSearch, setHasPendingUrlSearch] = useState(true);
  useEffect(() => {
    if (searchResults || searchLoading || searchError) {
      setHasPendingUrlSearch(false);
      return;
    }
    if (typeof window === 'undefined') return;
    const sp = new URLSearchParams(window.location.search);
    setHasPendingUrlSearch(Boolean(sp.get('from') && sp.get('to') && sp.get('date')));
  }, [searchResults, searchLoading, searchError]);

  // Loading — skeleton + centered spinner card
  if (searchLoading || hasPendingUrlSearch) {
  ...
  // Allocate hatası
  if (allocateError) {
    const isRateLimit = /too many requests/i.test(allocateError);
    const allocateErrorTitle = isRateLimit
  ...
  // Sonuç yok (arama tamamlandı ama 0 uçuş döndü, ya da URL parametreleri
  // olmadan doğrudan sayfaya gelinmişse fallback)
  if (!searchResults || searchResults.flights.length === 0) {
    return (
      <>
        <HeaderOne />
        <main className="bb-search-results">
          <div className="bb-empty-state bb-empty-state--boarding">
            <div className="bb-empty-bp">
              <div className="bb-empty-bp__left">
                <div className="bb-empty-bp__label">Kalkış</div>
                <div className="bb-empty-bp__code">{searchParams?.origin ?? '---'}</div>
                <div className="bb-empty-bp__city">{searchParams ? airportCity(searchParams.origin) : ''}</div>
              </div>
              <div className="bb-empty-bp__route">
                <div className="bb-empty-bp__dot bb-empty-bp__dot--red"></div>
                <div className="bb-empty-bp__line">
                  <svg viewBox="0 0 120 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    ...
                    <path d="M56 18L48 12l8-6v4h16v-4l8 6-8 6v-4H56v4z" fill="#94a3b8"/>
                    <line x1="80" y1="12" x2="120" y2="12" stroke="#e2e8f0" strokeWidth="1.5" strokeDasharray="4 3"/>
                  </svg>
                </div>
                <div className="bb-empty-bp__dot"></div>
              </div>
              <div className="bb-empty-bp__right">
                <div className="bb-empty-bp__label">Varış</div>
                <div className="bb-empty-bp__code">{searchParams?.destination ?? '---'}</div>
                <div className="bb-empty-bp__city">{searchParams ? airportCity(searchParams.destination) : ''}</div>
              </div>
            </div>
            <div className="bb-empty-state__divider"></div>
            <h2 className="bb-empty-state__title">Uçuş Bulunamadı</h2>
            <p className="bb-empty-state__text">
              Bu güzergahta seçili tarihlerde müsait sefer bulunamadı.
              Tarih aralığını genişletmeyi veya farklı bir rota denemeyi deneyin.
            </p>
            <div className="bb-empty-state__actions">
              <button className="bb-empty-state__btn bb-empty-state__btn--primary" onClick={() => router.push('/')}>
              ...
              {searchParams && (
                <button
                  className="bb-empty-state__btn bb-empty-state__btn--outline"
                  onClick={() => handleDateNav('dep-next')}
                >
  ...
  return (
    <>
      ...
      {/* ── Search sticky top: sibling of main so overflow-x:hidden on body doesn't break sticky ── */}
      {searchParams && (
        <div className="bb-search-sticky-top" ref={stickyTopRef}>
          <div className="bb-search-sticky-top__inner">
            <div className="bb-search-toolbar">
              <div className="bb-search-toolbar__chips">
                <span className="bb-search-toolbar__chip bb-search-toolbar__chip--route">
                ...
                <span className="bb-search-toolbar__chip">
                  ...
                  {searchParams.flightType === 'RT' && searchParams.returnDate && (
                    <> &mdash; {trDateShort(searchParams.returnDate)}</>
                  )}
                </span>
                <span className="bb-search-toolbar__divider" aria-hidden="true" />
                <span className="bb-search-toolbar__chip">
            ...
            <div id="bb-toolbar-panel-edit" role="region" aria-label="Aramayı düzenle" className="bb-toolbar-panel">
              <BannerFormOne />
            </div>
