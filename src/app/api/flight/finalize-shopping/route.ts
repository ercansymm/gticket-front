...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Session bilgisini önce cache'den al, yoksa backend'den çek
    ...
    const res = await fetch(`${API_BASE}/api/flight/finalize-shopping`, {
      method: 'POST',
      headers: {
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
      logger.error('Backend timeout', error, 'api/flight/finalize-shopping');
      return NextResponse.json({ error: 'Request timeout' }, { status: 504 });
    }
    logger.error('Finalize shopping failed', error, 'api/flight/finalize-shopping');
