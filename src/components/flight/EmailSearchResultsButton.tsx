...
// ── Inline SVG icons (avoids external icon dep) ──
        ...
        {status === "success" ? (
          <div className="p-8 text-center">
            ...
            <p className="text-sm text-slate-600">
              Arama sonuçları birkaç dakika içinde adresinize ulaşacaktır.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            <p className="text-sm text-slate-600">
              Uçuş arama sonuçlarınızı e-posta adresinize göndermek için aşağıdaki bilgileri doldurun.
            </p>
            ...
            <div>
              <label htmlFor="name-input" className="block text-sm font-medium text-[#1e293b] mb-1">
                Adınız <span className="text-slate-400 text-xs">(opsiyonel)</span>
              </label>
              <input
                id="name-input"
                type="text"
  ...
  return (
    <>
      <button
        ...
        aria-label="Arama sonuçlarını e-posta olarak gönder"
      >
...
// ── Inline SVG icons (keeps the component free of external icon deps) ──
const IconMail = ({ className = "" }: { className?: string }) => (
  <svg
    className={className}
    width="16"
...
export function EmailSearchResultsButton({
  ...
  const closeModal = useCallback(() => {
    setStatus((current) => {
      if (current === "loading") return current; // block close while sending
      setIsOpen(false);
      window.setTimeout(() => {
        setEmail("");
        setName("");
  ...
  return (
    <>
      <button
        type="button"
        onClick={() => setIsOpen(true)}
        className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-white border border-slate-200 text-[#0a1628] hover:bg-[#f8fafc] hover:border-[#047857] hover:text-[#047857] transition-colors duration-200 shadow-sm ${className}`}
        aria-label="Arama sonuçlarını e-posta olarak gönder"
      >
        <IconMail className="w-4 h-4" />
        <span>E-posta ile Gönder</span>
      ...
      {isOpen && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-[#0a1628] text-white">
              <div className="flex items-center gap-2">
                <IconMail className="w-5 h-5" />
                <h2 id="email-modal-title" className="font-semibold">
                  Sonuçları E-posta ile Al
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="flex items-center justify-end gap-2 pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={status === "loading" || !email.trim()}
                    className="inline-flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium text-white bg-[#047857] hover:bg-[#065f46] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {status === "loading" ? (
                      <>
                        <IconSpinner className="w-4 h-4 animate-spin" />
